...
export const CustomCard = ({iconName, title, label, data, isRow=false, columns}) => {
  return (
    <div style={{border: "1px solid #EBF5FD", boxShadow: "2px 2px 8px 0px #ABABAB29", borderRadius:"15px", width:"100%"}}>
        ...
        <div className={`d-flex ${isRow ? "flex-row": "flex-column detail-container"} gap-3 `} style={{padding:"15px", height:label === "Documents" && "180px", overflow: label === "Documents" && "scroll"}}>
            {
                data?.map((item)=>(
                    label === "parents" ?
